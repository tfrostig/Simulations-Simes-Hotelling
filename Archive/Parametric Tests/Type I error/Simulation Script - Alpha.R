...
## Loading Functions 
source('C:/Users/user/Google Drive/Simes Hotelling Paper/Simes Hoteling V2 - No HC/R Script/Full Simulation 21112018/Parametric Tests/Data Generation.R')
setwd('C:/Users/user/Google Drive/Simes Hotelling Paper/Simes Hoteling V2 - No HC/R Script/Full Simulation 21112018/Parametric Tests/Alpha Error')
...
## Name Vector 
Name.Vec <- c('Srivastava', 'Chen', 'Lopes', 'SH.Ln.LB', 'SH.Sn.LB', 'GCT', 'CLX', 'Simes', 'KNN')
L        <- length(Name.Vec)
## Run tests 
RunTests <- function(X, Y) { 
  ...
  temp.test <-c('Srivastava' = apval_Sri2008(X, Y)$pval,
                ...
                'SH.Ln.LB'    = SimesHotelingArma(mat.list, round(n/2) , p * log(p)),
                'SH.Sn.LB'    = SimesHotelingArma(mat.list, round(n/4) , p * log(p)),
                'GCT'   = GCT.test(X, Y, 25)$pvalue,
                'CLX'   = CLX_test(X, Y),
                'Simes' = t.simes(X, Y, var.equal = TRUE),
                'KNN'   = mtsknn(X, Y, round(n / 4))$`P Value`
