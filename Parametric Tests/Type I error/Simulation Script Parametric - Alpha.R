set.seed(998)
setwd('Parametric Tests/Type I error')

## Name Vector 
name.vec <- c('SD', 'CQ', 'Lopes', 'SH.Ln.LB', 'SH.Sn.LB', 'GCT', 'CLX', 'Simes') 
L        <- length(name.vec)
...
RunTests <- function(X, Y) { 
  ...
  temp.test <-c('SD' = apval_Sri2008(X, Y)$pval,
                'CQ'  = apval_Chen2010(X, Y)$pval,
                'Lopes' = lopesTest(X, Y, B1 = 1),
                'SH.Ln.LB'    = SHTest(X, Y, samp.size = round(n / 2) , 
                                       iterations = p * log(p))$`P-value`,
                'SH.Sn.LB'    = SHTest(X, Y, samp.size = round(n / 4) , 
                                       iterations = p * log(p))$`P-value`,
                'GCT'   = GCT.test(X, Y, 25)$pvalue,
                'CLX'   = CLX_test(X, Y),
                'Simes' = t.simes(X, Y, var.equal = TRUE)
                #'KNN'   = mtsknn(X, Y, round(n / 4))$`P Value` ### Removed since MTSKNN is not available for newer R versions 
...
for (i in 1:nrow(power.simulation.matrix.1)) { 
  ...
  temp.res <- foreach(j = 1:iter.num,
                      ...
                      .options.RNG = 9999) %dorng% { 
                        temp.dat  <-  createData(1, 
                                                 euclid.norm = 0 * sqrt(20 / temp.obs), 
                                                 group.num = 5, 
                                                 n.x = temp.obs, 
                                                 n.y = temp.obs, 
                                                 cov.mat.x = cov.mat, 
...
write.csv(power.simulation.matrix.1, 'alpha.simulation.matrix.1.csv', row.names = FALSE)
...
write.csv(power.simulation.matrix.3, 'alpha.simulation.matrix.3.csv', row.names = FALSE)
...
power.simulation.matrix.5 <- data.frame(cbind(expand.grid(false.hypo, obs, type), 
                                              matrix(NA ,ncol = L)))
...
for (i in 1:nrow(power.simulation.matrix.5)) {
  false.hypo.temp <- power.simulation.matrix.5[i, 'False Hypotheses']  
  obs.temp        <- power.simulation.matrix.5[i, 'Obs'] 
  temp.type       <- power.simulation.matrix.5[i, 'Type']
  ...
  print(paste('Iteration', i,'out of',nrow(power.simulation.matrix.5)))
}


write.csv(power.simulation.matrix.5,'alpha.simulation.matrix.5.csv', row.names = FALSE)
...
for (i in 1:nrow(power.simulation.matrix.7)) {
  ...
  temp.res <- foreach(j = 1:iter.num,
                      ...
                      .options.RNG = 9999) %dorng% { 
                        temp.dat        <- createData(1, 
                                                      euclid.norm = 0 * sqrt(20 / obs.temp), 
                                                      group.num = 5, 
                                                      n.x = obs.temp, 
                                                      n.y = obs.temp, 
                                                      cov.mat.x = cov.mat, 
  ...
  print(paste('Iteration', i,'out of',nrow(power.simulation.matrix.7)))
}

write.csv(power.simulation.matrix.7,'alpha.simulation.matrix.7.csv', row.names = FALSE)