...
set.seed(998)
setwd('Parametric Tests/Power')

## Name Vector 
name.vec <- c('SD', 'CQ', 'Lopes', 'SH.Ln.LB', 'SH.Sn.LB', 'Simes') 
L        <- length(name.vec)
...
RunTests <- function(X, Y) { 
  ...
  temp.test <-c('SD' = apval_Sri2008(X, Y)$pval,
                ...
                'SH.Ln.LB'    = SHTest(X, Y, samp.size = round(n / 2) , iterations = p * log(p))$`P-value`,
                'SH.Sn.LB'    = SHTest(X, Y, samp.size = round(n / 4) , iterations = p * log(p))$`P-value`,
...
colnames(power.simulation.matrix.1) <- c('False Hypotheses', 'Obs', 'Correlation', 'Type', 
                                         name.vec)
...
colnames(power.simulation.matrix.3) <- c('False Hypotheses', 'Obs', 'Block_Size', 'Rho', 'Type', 
                                         name.vec)
...
power.simulation.matrix.5 <- data.frame(expand.grid(false.hypo, obs, type), 
                                              matrix(NA ,ncol = L))
colnames(power.simulation.matrix.5) <- c('False Hypotheses','Obs', 'Type',
                                         name.vec)
...
power.simulation.matrix.7 <- data.frame(expand.grid(false.hypo, obs, correlation, type), 
                                        matrix(NA ,ncol = L))
colnames(power.simulation.matrix.7) <- c('False Hypotheses','Obs', 'Correlation','Type', 
                                         name.vec)
