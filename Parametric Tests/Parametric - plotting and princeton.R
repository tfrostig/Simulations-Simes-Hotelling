...
FacetPower <- function(transform.data, col.x, col.x.title, no.facet.flag = FALSE,plot.type = F,alpha = 0.05, iter.num = 1000) {
  plot.gg <- ggplot(transform.data, 
    ...
    scale_linetype_manual("Tests",
                          values = c('solid', 'solid', 'solid', 'longdash', 'longdash', 'solid'),
                          breaks = c('SD', 'CQ', 'Lopes', 'SH.Sn.LB', 'SH.Ln.LB', 'Simes'), 
                          labels = c("SD", "CQ", "Lopes", expression(SH[n/4]), expression(SH[n/2]), "Simes")) +
    guides(colour = guide_legend(override.aes = list(size = 0.85))) + theme_bw() + 
    # scale_colour_grey(start = 0.2, end = 1, labels = c("SD", "CQ", "Lopes", "SH.Ln.LB", "SH.Sn.LB","Simes")) + 
    scale_color_manual(values = c("#B15928", "#E31A1C", "#FDBF6F", "#6A3D9A" ,"#CAB2D6", "#33A02C"),
                       breaks = c('SD', 'CQ', 'Lopes', 'SH.Sn.LB', 'SH.Ln.LB', 'Simes'), 
                       labels = c("SD", "CQ", "Lopes", expression(SH[n/4]), expression(SH[n/2]), "Simes")) +
...
setwd(paste0(here::here(), '/Parametric Tests/Power'))

dat.1 <- read.csv('power.simulation.matrix.1.csv') ## AR(1)
dat.3 <- read.csv('power.simulation.matrix.3.csv') ## Block Covariance 
dat.5 <- read.csv('power.simulation.matrix.5.csv') ## Cai model 
dat.7 <- read.csv('power.simulation.matrix.7.csv') ## Equicorrelated 



### Directory to save plots 
setwd(paste0(here::here(), '/Parametric Tests/Images'))


# Plotting Setting 1 - AR(1) ----------------------------------------------
dat1.clean <- dat.1 %>% select(False.Hypotheses, Obs, Correlation, SD, CQ, Lopes, SH.Ln.LB, SH.Sn.LB, Simes) 
## Transforming Data 
...
plot.ar <- FacetPower(transform.data, 'Correlation')

ggsave('AR_Parametric.pdf', plot.ar, width = 7.5, height =8.5)
...
dat.3.size20 <- dat.3 %>% 
  ...
  select(c(False.Hypotheses, Obs, Rho, SD, CQ, Lopes, SH.Ln.LB, SH.Sn.LB, Simes)) %>%

transform.data <- dat.3.size20 %>% melt(id.vars = colnames(dat.3.size20)[1:3], 
                                        variable.name = 'Tests',  value.name = 'Power') %>% 
  mutate(Is.SH = str_detect(Tests,'SH'))


plot.3.size.20 <- FacetPower(transform.data, col.x = 'Correlation')

ggsave('Block20_Parametric.pdf', plot.3.size.20, width = 7.5, height =8.5)
...
ggsave('Block50_Parametric.pdf', plot.3.size.50, width = 7.5, height =8.5)
...
transform.data <- dat.3.size100 %>% melt(id.vars = colnames(dat.3.size100)[1:3],
                                         variable.name = 'Tests',  value.name = 'Power') %>% 
  mutate(Is.SH = str_detect(Tests,'SH'))

plot.3.size.100 <- FacetPower(transform.data, col.x = 'Correlation')

ggsave('Block100_Parametric.pdf', plot.3.size.100, width = 7.5, height =8.5)


# Plotting Setting 7 - Equicorrelated Covariance --------------------------

equicorr.dat <- dat.7 %>% 
  select(c(False.Hypotheses, Obs, Correlation, SD, CQ, Lopes, SH.Ln.LB, SH.Sn.LB, Simes))

transform.data <- equicorr.dat %>% melt(id.vars = colnames(equicorr.dat)[1:3], 
                                        variable.name = 'Tests',  value.name = 'Power') %>% 
  mutate(Is.SH = str_detect(Tests,'SH'))

plot.equicorr <- FacetPower(transform.data, col.x = 'Correlation')

ggsave('Equicorr_Parametric.pdf', plot.equicorr, width = 7.5, height =8.5)




# Plotting Setting 5 - Model Cai 7 ----------------------------------------
dat.5.clean <- dat.5 %>% select(c(False.Hypotheses, Obs, SD, Chen, CQ, SH.Ln.LB, SH.Sn.LB, Simes))

transform.data <- dat.5.clean %>% melt(id.vars = colnames(dat.5.clean)[1:2],
                                       variable.name = 'Tests',  value.name = 'Power') %>% 
  mutate(Is.SH = str_detect(Tests,'SH'))

plot.cai.model.7 <- FacetPower(transform.data, col.x = 'False.Hypotheses', no.facet.flag = TRUE)

ggsave('CaiModel7_Parametric.pdf', plot.cai.model.7, width = 7.5, height =8.5)
...
power.test.obs20 <- power.df %>% 
  filter(Obs == 20) %>% 
  select(c(SD, CQ, Lopes, SH.Ln.LB, SH.Sn.LB, Simes)) 
...
write.csv(prince.df, 'parametric.ala.princeton.csv')
